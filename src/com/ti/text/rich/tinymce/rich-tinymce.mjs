const _M = {
  ...
  computed: {
    ...
    TheTinyEditor() {
      ...
      let plugins = _.concat(
        "paste lists advlist table searchreplace",
        plugNames,
        tinyPlugins
      );
      //.........................................
  ...
  methods: {
    ...
    OnHeadingChange($node, { resetOutlineId = false } = {}) {
      let $h = Ti.Dom.closest(
        $node,
        (el) => {
          return (
            /^(sub-)?title$/.test(el.getAttribute("doc-heading")) ||
    ...
    async initEditor() {
      ...
      //console.log("extended_valid_elements", extended_valid_elements);
